...
try {
    ...
    if (!in_array($role, ['super_admin', 'admin'])) {
        ...
        echo json_encode(["error" => "Access Denied. Only super_admin or admin can view dustbin assignments."]);
        exit;
    }
} catch (Exception $e) {
    http_response_code(401);
...
// Fetch dustbin assignments for the given dustbin_id
$stmt = $conn->prepare("SELECT card_id, emp_name, department, designation, dustbin_id, location FROM dustbin_assignments WHERE dustbin_id = ?");
...
logEvent("User with email $email accessed dustbin assignments for dustbin ID: $dustbin_id");

if (empty($assignments)) {
    http_response_code(404);
    echo json_encode(["error" => "No assignments found for this dustbin"]);
} else {
    echo json_encode(["dustbin_assignments" => $assignments], JSON_PRETTY_PRINT);
}
